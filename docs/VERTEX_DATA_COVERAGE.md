...
| Dossier : chaîne, grille, surface, max pain | chaîne chargée **en fond** (`chaine_a_la_demande`), `en_cours` servi, page qui réessaie ; blocs IBKR vides sans TWS (dit) | RÉEL / PARTIEL | corrigé (réseau hors requête) |
...
| Performance (équité, contribution) | équité dérivée des clôtures déclarées (exit − coût cumulé, base = capital) ; `plAbs` produit par `enrich` → contribution alimentée dès qu'une marque existe | RÉEL (déclaratif) | corrigé |
...
| 8 | P1 | Réseau dans les requêtes UI (corrélations, descriptions, analystes, dossier options, pos-quotes) | **partiel** : `/api/analyst` sert le cache (CACHE / PERIME + `stale`) et collecte en fond (EN_COURS + `retry_s`, une collecte par symbole), la fiche réessaie 3× ; `/api/pos-quotes` n'attend plus que 1,5 s (`POSQ_ATTENTE_S`) et nomme `en_attente`, son repli lit la marque de contrat depuis le cache (`contract_mark(reseau=False)`, lecture en fond). Routes du dossier options (`/api/options/chain`, `/strategies`, `/options-for`, intel) : `chaine_a_la_demande.board_avec` (magasin non bloquant, collecte en fond) remplace `on_demand.board_with` ; `en_cours` + `retry_s` servis, pages Options avec réessai borné (gardien : plus aucun `_od.board_with`/`_od.fetch` dans les routes). `/api/company` (cache + collecte en fond, CACHE/PERIME/EN_COURS) et `/api/correlations` (calcul en fond par titre, cache TTL 1 h, EN_COURS) fermés aussi — **clos** |
| 9 | P1 | Composants jamais alimentés (changes_since_prev, daily_changes, #an-committee, priceDomain, équité portefeuille) | **partiel** : `#an-committee` et `#an-catalyst-strip` ont leur hôte (le raisonnement du comité s'affiche dans la fiche), `priceDomain`/`status` déclarés (âge réel de la cotation) ; les blocs analystes de la fiche (surprises, révisions, notes, détenteurs, initiés) étaient peints à 1 s puis EFFACÉS à 2 s par la réécriture du corps de carte (mesuré) — ajout idempotent `data-analyst` ; « Ce qui a changé » branché sur `/api/market/context` (`changes_base`, `prev_as_of`, `as_of`) au lieu du contexte brut du scan qui ne porte jamais de diff ; équité Portefeuille dérivée des clôtures déclarées ; `plAbs` produit ; `daily_changes` (jamais produit) remplacé dans le brief par le diff de market_context — **clos** |
...
| 15 | P1 | Diffusion : émetteurs SSE présents (scan, options, alertes) mais aucune réaction côté pages hors deux cas — toutes les cartes sondaient | **corrigé** (P1) : émetteurs actualités et jobs ajoutés ; le client invalide le cache et rejoue les tâches de la page sur événement (différé 1,5 s, rattrapage en onglet masqué, aucun reload) ; Marchés, Opportunités (hors screener) et Portefeuille enregistrent leurs chargeurs |
| 16 | P1 | Actualités : horodatage de publication brut et hétérogène (IBKR/yfinance/RSS), aucune heure de réception, fil sans époque serveur, nombre de sources non montré | **corrigé** (P2) : `published_at` normalisé par la source (jamais inventé, None si illisible), `received_at` UTC, `news_state.ts`/`as_of`, carte Aujourd'hui avec heure de publication, info-bulle de réception et « N sources » ; déduplication multi-sources déjà en place, gardée par test |
| 17 | P1 | Gardiens navigateur (8 espaces à 390 px, contraste, règles CSS mortes, couche visuelle, boutons morts) qui s'abstenaient : binaire Chromium absent, puis `chrome.exe` complet impossible à engendrer sur cette machine, cible figée sur l'instance de travail (5002, code d'accès) | **corrigé** (P4) : Chromium installé (cache Playwright utilisateur), la sonde préfère le shell headless, `VERTEX_MESURE_BASE` cible l'instance QA ; **53 gardiens passent réellement**. Deux défauts mesurés et corrigés : Système › bouclier READONLY débordait de 44 px à 390 px ; Aujourd'hui › cartes positions en 4,46:1 (< 4,5) |
| 18 | P2 | Aucune publication européenne ou suisse (communiqués de banques centrales) dans l'application | **corrigé** (P5) : flux RSS officiels BCE (communiqués de presse) et BNS (ad hoc) collectés par le collecteur macro (6 h), parseur durci, dédoublonnage par lien, `published_at` de la source, carte Marchés › Macro « Communiqués officiels » ; fixtures réelles capturées, collecte réelle vérifiée (24 communiqués) |
| 14 | P2 | Onglets Options : neuf onglets servis, sept sous-vues au contrat | consigné — proposition de regroupement dans `VERTEX_FINAL_REPORT.md` §9, décision humaine (inverse la décision « plus aucune vue orpheline ») |
