...
| Exécution | Résultat |
|---|---|
| Suite complète finale (`python -m pytest -q`) | **4422 passed, 181 skipped, 0 failed** (3 min 10 s) |
...
| Nouveaux tests | `test_ibkr_session_marche_seule` (63), `test_macro_officiel` (12), `test_etiquettes_live_honnetes` (4), `test_fraicheur_serveur` (29) |
...
Voir `VERTEX_DATA_COVERAGE.md` §13 : verdict `ATTENDRE` fabriqué hors scan et
trois autorités de décision (P0, programme lot décision) ; calendrier
« Confirmé » systématique côté page (le serveur fournit désormais
`confirmation`, la page ne le lit pas encore) ; réseau dans les requêtes UI
(corrélations, descriptions, analystes, dossier options, pos-quotes) ; risque
du panier calculé sur le comité et non sur le portefeuille ; composants jamais
alimentés (« Ce qui a changé », `#an-committee`, puce prix, équité
portefeuille) ; verdict/liquidité calculés dans le JS de la vue Structure ;
Cerveau Claude qui affiche des cotations web non réconciliées ; onglets
