...
self.addEventListener('fetch',e=>{
  ...
  e.respondWith((async()=>{
    ...
    try{
      ...
      // `ignoreSearch` : le pre-cache stocke `/asset/css/bundle.css` SANS
      // requete, la page demande `?v=vx-shell-N`. Sans cette seconde
      // tentative, le repli hors-ligne rendait une coque NUE (ni style ni
      // JS, donc pas meme le voyant hors-ligne). Mesure le 2026-09-06.
      const c=(await cache.match(req))||(await cache.match(req,{ignoreSearch:true}))||(req.mode==='navigate'?await cache.match('/'):null);
      return c||fetch(req);
    }
  })());
});
